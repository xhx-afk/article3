...
def save_on_master(obj, path, *args, **kwargs):
    ...
    try:
        ...
        # Preserve torch.save support for file-like objects.
        torch.save(obj, path, *args, **kwargs)
        return

    suffix = f'.tmp.{os.getpid()}'
    temporary = target + (suffix.encode() if isinstance(target, bytes) else suffix)
    try:
        torch.save(obj, temporary, *args, **kwargs)
